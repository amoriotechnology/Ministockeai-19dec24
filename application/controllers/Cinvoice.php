...
$data['attach'] = $getAttach;
 ...
 public function getvendor_products() {
    ...
    $filename = 'Estimate_' . $purchase_detail[0]['purchase_id'] . '.pdf';
    if (empty($pdf)) {
...
public function manual_sales_insert(){
 ...
 public function salesIndexData() {
        ...
        $items          = $this->Invoices->getPaginatedsales($limit, $start, $orderField, $orderDirection, $search, $date);
        $sale   = $this->Invoices->newsale($date);
        $email_settings = $this->Invoices->getEmailConfigdata();
        $currency_details = $this->Web_settings->retrieve_setting_editdata();
        $totalItems     = $this->Invoices->getTotalsales($search, $date);
        $data           = [];
        $i              = $start + 1;
        $edit           = "";
        $delete         = "";
        ...
        foreach ($items as $item) {
            $action = "";
            $action .= "<a href='".base_url('Cinvoice/invoice_inserted_data/'.$item['invoice_id'])."' class='btnclr btn btn-success btn-sm'> <i class='fa fa-download'></i> </a>
                        <a class='btnclr btn btn-sm getinvoice_id' data-toggle='modal' data-target='#emailmodal' onclick='mail(".$item['invoice_id'].",&apos;invoice&apos;,&apos;invoice_id&apos;)'><i class='fa fa-envelope' aria-hidden='true' ></i></a>";

            foreach($this->session->userdata('perm_data') as $test) {
                $split = explode('-',$test);
                if(trim($split[0])=='sales' && $_SESSION['u_type'] ==3 && trim($split[1])=='0010') {
                    $action .= "&nbsp;<a class='btnclr btn btn-sm invoice_edit' href='Cinvoice/invoice_update_form/'".$item['invoice_id']."'><i class='fa fa-pencil' aria-hidden='true'></i></a>";
                } 
                if(trim($split[0])=='sales' && $_SESSION['u_type'] ==3 && trim($split[1])=='0001') {
                    $action .= "&nbsp;<a class='btnclr btn btn-sm' onclick='return confirm(\"" . display('are_you_sure') . "\")' href='Cinvoice/sale_invoice_delete/'".$item['invoice_id']."/".$item['commercial_invoice_number']."'><i class='fa fa-trash-o' aria-hidden='true'></i></a>";
                } 
            }

            if ($_SESSION['u_type'] == 2) {
                $invoice_id = htmlspecialchars($item['invoice_id'], ENT_QUOTES, 'UTF-8');
                $commercial_invoice_number = htmlspecialchars($item['commercial_invoice_number'], ENT_QUOTES, 'UTF-8');
                $action .= "&nbsp;<a href='" . base_url('Cinvoice/invoice_update_form/' . $invoice_id) . "' class='btnclr btn btn-sm invoice_edit'>
                                <i class='fa fa-pencil' aria-hidden='true'></i>
                            </a>
                            <a href='" . base_url('Cinvoice/sale_invoice_delete/' . $invoice_id . "/" . $commercial_invoice_number) . "' class='btnclr btn btn-sm' onclick='return confirm(\"" . display('are_you_sure') . "\")'>
            ...
            $row = [
                "sno"      => $i,
                "invoice"      => $item['commercial_invoice_number'],
                "cust_name"    => $item["customer_id"],
                "sale_inv_date" => $item['date'],
                "tot_amt"   => '$'.$item['gtotal'],
                "amt_paid"   => '$'.(!empty($item['paid_amount']) ? $item['paid_amount'] : 0),
                "action" => $action,
            ];
            $data[] = $row;
            $i++;
   ...
   //print_r($data);
    ...
    public function invoice_inserted_data($invoice_id) {
        ...
        $data=array(
            ...
            'tax_des'         => $invoice_detail[0]['total_tax'],            
            'landing_cost'    => $invoice_detail[0]['landing_cost'],
