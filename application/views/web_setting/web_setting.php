               ...
               <div class="row">
                  ...
                  <div class="col-md-6">
                     <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                           <input class="form-control" name ="footer_text" id="footer_text" type="text" placeholder="Footer Text" value="{footer_text}" tabindex="7">
                        </div>
                     </div>
                  </div>
                  <div class="col-md-6">
        ...
        <input type="hidden" class="txt_csrfname" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>">
         ...
         <div class="modal-body">
            <table class="table table-bordered" id="tableAlerts" >
               ...
               <tbody>
                <!-- <form action="<?php echo base_url('Cweb_setting/sendAlerts') ?>" method="post"> -->
                <?php echo form_open_multipart('Cweb_setting/sendAlerts',array('class' => 'form-vertical','id' => 'validate' ))?>
                  <tr>
                     <td> <input type="checkbox" id="eta" name="checkbox" /></td>
                     <td><span id="bold"><?php echo display('NEW SALE') ?></span> - ETD</td>
                     <td>
                        <select class="when select_date" name="select_date" style="width: -webkit-fill-available;" required>
                        ...
                        <!-- <input type="hidden" class="sch_id" value="<?php echo $scheduleData[0]->id; ?>"> -->
                     </td>
                     <td style="display: none;"><input type="hidden" name="status" class="ETD" value="NewSaleETD"></td>
                     <td>
                        <select class="where select_source" name="select_source"  style="width: -webkit-fill-available;" required>
                ...
                <?php echo form_close()?>
                <?php echo form_open_multipart('Cweb_setting/sendAlerts',array('class' => 'form-vertical'))?>
                  <tr>
         ...
         <!-- <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary save_datain" style="background-color:#1c2350;"><?php echo display('Save') ?></button>
      ...
      <div class="modal-content" style="width: max-content;">
         ...
         <div class="modal-body">
            <table class="table table-bordered" >
               ...
               <tbody>
                  <?php echo form_open_multipart('Cweb_setting/sendAlertsexpense',array('class' => 'form-vertical'))?>
                  <tr>
                     ...
                     <td class="EMAIL_exp">
                        <select class="where_exp" name="email" name="select_email" style="width: -webkit-fill-available;">
                           <option value="Select Preferred Email"><?php echo display('Select Preferred Email') ?></option>
                        </select>
                     </td>
                  ...
                  <tr>
                     ...
                     <td><span id="bold"><?php echo display('PURCHASE ORDER') ?></span> - <?php echo display('ETD') ?></td>
                     <td>
                        <select class="when_exp" name="select_date" style="width: -webkit-fill-available;">
                           <option value="Select Preferred Date"><?php echo display('Select Preferred Date') ?></option>
                        </select>
                     </td>
                     <td style="display: none;"><input type="hidden" name="status" value="Estshipmentdate"></td>
                     <td>
                        <select class="where_exp" name="select_source" style="width: -webkit-fill-available;">
                           <option value="Select Preferred Source"><?php echo display('Select Preferred Source') ?></option>
                        </select>
...
<script>
    // 1 st New sale ETD
    $(document).ready(function(){
        $('.save_datain').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_date').val(); 
         var selectSource = $('.select_source').val();  
         var status = $('.ETD').val(); 
         
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            dataType: 'json',
            data: {[csrfName]: csrfHash,selectDate:selectDate,selectSource:selectSource, status: status},
    ...
    // 2 nd New sale ETA
    $(document).ready(function(){
        $('.save_dataetd').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_dateeta').val(); 
         var selectSource = $('.select_sourceeta').val();   
         var status = $('.ETA').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
    ...
    // 3 rd 
    $(document).ready(function(){
        $('.save_datapdd').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_datePaymentduedate').val(); 
         var selectSource = $('.select_sourcePaymentduedate').val();   
         var status = $('.PAYMENTDUEDATE').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            // dataType: 'json',
    ...
    // 4 th 
    $(document).ready(function(){
        $('.save_dataoctd').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_dateOCEANEXPORTETD').val(); 
         var selectSource = $('.select_sourceOCEANEXPORTETD').val();   
         var status = $('.OCEANETD').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            // dataType: 'json',
    ...
    // 5 th 
    $(document).ready(function(){
        $('.save_dataoceta').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_dateOCEANEXPORTETA').val(); 
         var selectSource = $('.select_sourceOCEANEXPORTETA').val();   
         var status = $('.OCEANETA').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            // dataType: 'json',
    ...
    // 6 th 
    $(document).ready(function(){
        $('.save_datatruck').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_dateTRUCKINGCPD').val(); 
         var selectSource = $('.select_sourceTRUCKINGCPD').val();   
         var status = $('.TRUCKINGPICKUPDATE').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            // dataType: 'json',
    ...
    // 7 th 
    $(document).ready(function(){
        $('.save_datatruckDD').click(function(event){
        event.preventDefault()
         var selectDate = $('.select_dateTRUCKINGDD').val(); 
         var selectSource = $('.select_sourceTRUCKINGDD').val();   
         var status = $('.TRUCKINGDELIVERYDATE').val();  
         $.ajax({
            url:"<?php echo base_url(); ?>Cweb_setting/sendAlerts",
            type: 'POST',
            // dataType: 'json',
    ...
    // $(document).ready(function(){
    //     $('.get_companylogo').click(function(event){
    //     event.preventDefault()
    //      var company_logo = $('.c_logo').val(); 
    //      alert(company_logo); 
    //      var hidden_logo = $('.old_officelogo').val();   
    //      $.ajax({
    //         url:"<?php echo base_url(); ?>Cweb_setting/updatecompanyLogo",
    //         type: 'POST',
    //         // dataType: 'json',
    //         data: {[csrfName]: csrfHash,company_logo:company_logo,hidden_logo:hidden_logo},
    //         success:function(data){
    //             console.log(data);
    //             // location.reload();
    //         }
