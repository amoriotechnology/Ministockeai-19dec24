...
class Invoices extends CI_Model {
    ...
    public function getPaginatedsales($limit, $offset, $orderField, $orderDirection, $search, $date = null)
    {
        ...
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like("a.commercial_invoice_number", $search);
            $this->db->or_like("a.customer_id", $search);
            $this->db->or_like("a.date", $search);
            $this->db->group_end();
        }
    ...
    public function getTotalsales($search, $date = null)
    {
        $this->db->select('a.*,b.customer_name');
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id','left');
 ...
 public function container_booking_no(){
   ...
   public function invoice_entry() {
    ...
    $datainv = array(
        ...
        'date'            => date('m-d-Y', strtotime($this->input->post('invoice_date',TRUE))),          
