...
class Attendance_model extends CI_Model {
    ...
    public function __construct() {
        parent::__construct(); $this->db=$this->load->database('mini_stockeai',true);
    }
  ...
  public function atten_create($data = array())
    ...
    public function attendance_list() {
        $date = date('Y-m-d');
        $query =$this->db->select("count(DISTINCT(e.att_id)) as att_id,e.*,p.id,p.first_name,p.last_name")->join('employee_history p','e.employee_id = p.id','left')->where('e.date',$date)->where('e.create_by',$this->session->userdata('user_id'))->group_by('e.att_id')->order_by('e.att_id', 'desc')->get('attendance e');
 ...
 public function designation_dropdown(){
    ...
    public function attendance_data_checkin($id){
        ...
        $this->db->where('e.create_by',$this->session->userdata('user_id'));
        $this->db->where('e.att_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
...
function search($id,$start_date,$end_date)
        ...
        function search_intime($date,$start_time,$end_time)
    {
        ...
        $this->db->select('e.*,p.id,p.first_name,p.last_name');
        $this->db->from('attendance e');
        $this->db->join('employee_history p','e.employee_id = p.id','left');
        $this->db->where('e.create_by',$this->session->userdata('user_id'));
